...
import { resolveSchema as resolveUntypedSchema, generateMarkdown, generateTypes } from 'untyped'
    ...
    async function resolveSchema () {
      ...
      for (const layer of nuxt.options._layers) {
        ...
        if (filePath && existsSync(filePath)) {
          ...
          const schema = await resolveUntypedSchema(loadedConfig, lastLayerDefaults)
          schemas.push(schema)
        }
      }
      ...
      const userSchema = await resolveUntypedSchema(nuxt.options.$schema, mergedSchema.default as any)
